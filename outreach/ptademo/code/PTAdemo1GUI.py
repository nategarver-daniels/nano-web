...
var_timeseriesfilename.set("m200aSinusoid")
...
ax_profile1 = fig_profile.add_subplot(111) #just for show
...
## ----------
## Pulse Data and Profile Filename Entry Text
## ----------
...
label_estimates = Tk.Label(frame_entry,text="INITIAL ESTIMATES")
label_estimates.grid(row=5,column=1)

label_fits = Tk.Label(frame_entry,text="BEST-FIT VALUES")
...
ax_residual1 = fig_residual.add_subplot(111) #just for show
...
def redraw_axes():
    ...
    ax_profile1.set_xlabel("time (sec)")
    ax_residual1.set_xlabel("time (sec)")
...
def func_calresiduals():
    ...
    global ts, T1, residuals1, errorbars1
 
    # calculate residuals
    var_message.set("calculating residuals...")
    root.update()
...
def func_fitsinusoid():
    ...
    global residuals1, errorbars1, yfit1
    ...
    # best-fit sinusoid
    tfit = np.linspace(0, residuals1[-1,0], 1024)
...
button_loadprofiles = Tk.Button(frame_button,text="Load pulse profile",command=lambda: func_loadprofiles())
...
button_fitsinusoid = Tk.Button(frame_button,text="Fit sinusoid & remove offset",command=lambda: func_fitsinusoid())
button_fitsinusoid.grid(row=0,column=4)
